...
int Game::noise2(int x, int y) {
    int tmp = hash[(y + seed) % 256];
    return hash[(tmp + x) % 256];
}

float Game::lin_inter(float x, float y, float s) {
    return x + s * (y-x);
}

float Game::smooth_inter(float x, float y, float s)
{
    return lin_inter(x, y, s * s * (3-2*s));
}

float Game::noise2d(float x, float y)
{
    int x_int = x;
    int y_int = y;
...
float Game::perlin2d(float x, float y, float freq, int depth)
{
    float xa = x*freq;
    float ya = y*freq;
    float amp = 1.0;
